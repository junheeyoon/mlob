...
import com.asianaidt.ict.analyca.service.schedulerservice.config.PersistableCronTriggerFactoryBean;
...
class JobUtil {
    ...
    protected static JobDetail createJob(Class<? extends QuartzJobBean> jobClass, boolean isDurable,
                                         ApplicationContext context, String jobName, String jobGroup, Date endDate, Schedule.sequentialStatus sequential) {
...
//        jobDataMap.put("scheduleId", jobId);
        jobDataMap.put("endDate", endDate);

        if (sequential == Schedule.sequentialStatus.SEQUENTIAL)
            jobDataMap.put("sequential", "sequential");
        else
//            jobDataMap.put("sequential", "concurrent");
            jobDataMap.put("sequential", "parallel");
